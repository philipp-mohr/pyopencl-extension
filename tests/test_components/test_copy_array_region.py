...
def test_copy_array_region_on_device(cl_init):
    ary_np = np.array([[1, 2, 3], [4, 5, 6]], dtype=np.complex64)
    in_cl = cl.array.to_device(cl_init.queue, ary_np)
...
def test_copy_array_region_on_device_region_none(cl_init):
    ary_np = np.array([[1, 2, 3], [4, 5, 6]], dtype=np.complex64)
    in_cl = cl.array.to_device(cl_init.queue, ary_np)
...
def test_copy_array_region_on_device_between_buffers(cl_init):
    ary_np = np.array([[1, 2, 3], [4, 5, 6]], dtype=np.complex64)
    in_cl = cl.array.to_device(cl_init.queue, ary_np)
    out_np = np.zeros(shape=(4, 4), dtype=in_cl)
    out_cl = cl.array.to_device(cl_init.queue, out_np)
...
def test_copy_array_region_on_device_negative_indexing(cl_init):
...
def test_copy_array_region_on_device_given_axis_index(cl_init):
    ary_np = np.ones(shape=(5, 10), dtype=np.int32)
    in_cl = cl.array.to_device(cl_init.queue, ary_np)
    copy_region = CopyArrayRegion(in_buffer=in_cl,
                                  region_in=Slice[2:-1, :])
...
# todo
# def test_step_width_larger_one(cl_init):
#     ary_np = np.array([np.arange(10, dtype=np.int32)] * 2)
#     in_cl = cl.array.to_device(cl_init.queue, ary_np)
#     out_cl = cl.array.to_device(cl_init.queue, ary_np)
#     copy_region = CopyArrayRegion(in_buffer=in_cl,
#                                   region_in=Slice[:, ::2],
